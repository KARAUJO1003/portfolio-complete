...
| Redesign do admin e login | in-progress | Historico detalhado em `docs/admin-visual-references.md`. Nav principal (2026-07-17): `UserMenu` (avatar com dropdown: Minha conta/Site publico/tema/Sair sempre visivel) + grupos da nav viraram mega-menu por hover (`components/ui/menu.tsx`, Base UI, ref. `reui.io` navbar-12), sem mais scroll horizontal. Revisao completa tela a tela pela ordem do menu concluida (2026-07-17): Inicio (StatGrid/Stat em vez de cards soltos), Perfil, Minha conta (FormSection redundante removido), Projetos, Habilidades, Trajetoria, Paginas, Secoes customizadas (ganhou tabela de verdade, nao tinha), Usuarios - todos com `DataTableFrame` + `Drawer` bottom + acao no header + `FormSection` sem card; Portfolio/Curriculo Builder (janela de navegador / folha de papel); Design system atualizado para refletir tudo isso (secao Menu nova, legendas corrigidas). Pendente para depois (nao pedido ainda): fase 2 de densidade em `table.tsx`/`textarea.tsx`/`checkbox.tsx`/`badge.tsx`/`form.tsx` (FormAside/FormPreviewFrame), nav em mobile, imagem de referencia Vercel/Supabase (usuario ainda nao enviou). Ajustes de feedback ao vivo (2026-07-17, continuacao): cor do Textarea/RichTextEditor alinhada ao Input, bug sistemico de icone `size-3.5` no `Button` corrigido para `size-4`, `PageTitle` reduzido, `NumberField` sem divisorias (`components/ui/number-field.tsx`), `DrawerFormShell` (`w-full` com conteudo `max-w` interno). Todo `<select>` nativo trocado por `components/ui/select.tsx` (Base UI, particle `@coss/select`) - incluiu um bug de Select nao abrir dentro de Drawer (foco disputado entre `alignItemWithTrigger` do Base UI e o focus-trap do vaul; fix: `alignItemWithTrigger={false}`), documentado em `docs/admin-visual-references.md` |
| Sistema avancado de formularios admin | in-progress | `projects` continua sendo o piloto completo (steps, preview, aside); agora vive num `Drawer` bottom (nao mais inline na pagina) - decisao 2026-07-17: forms com aside/preview usam Drawer bottom (largura toda), Sheet lateral (`components/ui/sheet.tsx`, novo) fica para forms simples de coluna unica, ainda sem uso real. `skills`, `experiences`, `pages`, `profile` e `custom-sections` seguem no padrao basico inline (`DsForm`/`FormSection`/`FormFields`), sem preview/steps. `custom-sections` ganhou form dedicado com React Hook Form (antes era `useState` puro). Tiptap/autocomplete ficam para fase seguinte |
| Tabelas admin padronizadas | in-progress | `DataTableFrame` criado e piloto em projetos com busca, filtro e ordenacao; falta replicar nos demais CRUDs. Hardening (2026-07-17): `ConfirmDialog` antes de excluir e `ErrorState` em erro de listagem em todas as tabelas; skeleton real de loading (`DataTable` ganhou prop `isLoading`); corrigido `overflow-hidden` que cortava a coluna Acoes no mobile (agora `overflow-x-auto`); foco visivel padronizado em Input/Textarea/selects nativos. Correcao adicional 2026-07-17: busca/filtro do `DataTableFrame` agora e irmao acima do frame, nao filho (bug documentado em `docs/admin-visual-references.md` corrigido); Projetos ganhou vista em grade (`ProjectCard`) alternavel com a tabela |
...
**Checkpoint de retomada (2026-07-17):** as 4 telas piloto do redesign (Projetos, Skills, Portfolio Builder, Curriculo Builder) estao **implementadas em codigo React de verdade** (nao so mockup HTML) e testadas no navegador: `DataTableFrame` corrigido, toggle grade/tabela em Projetos, tabela unica com filtro de categoria em Skills, forms migrados para `Drawer` bottom com acao principal no header (toolbar) em vez de rodape flutuante, `FormSection` sem card aninhado, tema shadcn "Mira" aplicado so ao admin via `.admin-shell` (portfolio publico intocado, bug de portal escapando do escopo corrigido), Portfolio Builder com chrome de janela de navegador, Curriculo Builder com folha de papel sobre tray `surface-muted`. Ver `docs/design-references/admin-redesign-direction.html`/`admin-redesign-pages.html` (mockups originais, historico) e `docs/admin-visual-references.md` (decisoes reais aplicadas, com secoes datadas 2026-07-17). Proximo passo ao retomar: seguir para Experiencias, Paginas, Secoes e Usuarios com o mesmo processo (mockup ja nao e mais pre-requisito - os padroes de Projetos/Skills servem de referencia direta de codigo). Navegacao top nav segue bloqueada ate o usuario enviar a imagem de referencia do Vercel/Supabase.
