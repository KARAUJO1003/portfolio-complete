...
/**
 * Split-screen compartilhado pelas 3 telas de autenticacao (login, esqueci
 * minha senha, redefinir senha) - antes so o login tinha esse tratamento;
 * forgot/reset-password eram so um Card centralizado num fundo liso. Extraido
 * do antigo `app/login/page.tsx` pra evitar 3 copias da mesma marcacao.
 * Ver docs/admin-redesign-tasks.md, Fase 6.
 */
export function AuthShell({ children }: { children: ReactNode }) {
  return (
    <main className="admin-shell relative grid min-h-dvh overflow-hidden bg-background-subtle text-foreground lg:grid-cols-[minmax(0,1fr)_480px]">
      ...
      <section className="flex min-h-dvh items-center justify-center px-4 py-10">{children}</section>
    </main>
  );
}
