...
export function ProjectLikeButton({ compact = false, initialLikesCount, projectId }: ProjectLikeButtonProps) {
  ...
  return (
    <span className={compact ? "relative inline-flex" : "inline-flex"}>
      <motion.button
        aria-label={label}
        aria-pressed={liked}
        className={
      ...
      {compact ? (
        <motion.span
          animate={{ opacity: 1, scale: 1 }}
          aria-hidden="true"
          className="pointer-events-none absolute -bottom-1 -right-1 min-w-[18px] rounded-full border border-border bg-background px-1 text-center font-mono text-[10px] font-medium leading-[16px] text-foreground shadow-sm"
